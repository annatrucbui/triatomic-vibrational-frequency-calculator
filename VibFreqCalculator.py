"""
Main program for the Vibrational Frequency Calculator for Triatomic

Input: Takes a directory pathname (relative path)
Generate:
...
Usage: $python3 VibFreqCalculator.py
...
def main():
    ...
    name = pathname.split("outfiles")[0][-3:]
    ...
    # Starting from 10 number of points away from equilibrium, optimise  
    # Stop until the error in fitting is smaller than the desired tolerance
    
    print("\n Fitting curve around minimum along r:")
    errorlimit_r = 0.05 # tolerace of 5%
    for i in range(10):
        try:
    ...
    errorlimit_theta = 0.02 # tolerance of 2%
    for i in range(10): 
        try:
